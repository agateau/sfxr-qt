...
bool Audio::LoadSettings(char* filename) {
...
bool Audio::SaveSettings(char* filename) {
...
void Audio::ResetSample(bool restart) {
    if (!restart) {
        phase = 0;
    }
...
void Audio::SynthSample(int length, float* buffer, FILE* file) {
...
bool Audio::ExportWAV(char* filename) {
...
static void SDLAudioCallback(void* userdata, Uint8* stream, int len) {
    ...
    audio->playCallback(stream, len);
}

void Audio::playCallback(unsigned char* stream, int len) {
